defmodule SampleScenicDriverWaveshare.Scene.Main do
  use Scenic.Scene
  ...
  @splash_hash Scenic.Cache.Support.Hash.file!(
                 :code.priv_dir(:sample_scenic_driver_waveshare) |> Path.join(@splash_path),
                 :sha
               )
  @splash_width 128
  @splash_height 128
  ...
  def handle_input(
    ...
    Logger.debug(
      "SampleScenicDriverWaveshare.Scene.Main.handle_input: received event #{inspect(event)} state=#{
        inspect(state)
      }"
    )
  ...
  def handle_input(event, _context, state) do
    Logger.info(
      "SampleScenicDriverWaveshare.Scene.Main.handle_input: received event #{inspect(event)}"
    )
